blockTools =  require('@sanity/block-tools').default
...
const HTMLdocument = `<html><body><p>You might think you’re writing JSX:</p>
...
<p>That works fine, except when your <code>message.text</code> is something like <code>'&lt;img src onerror=&quot;stealYourPassword()&quot;&gt;'</code>. <strong>You don’t want things written by strangers to appear verbatim in your app’s rendered HTML.</strong></p>
...
<p>If <code>message.text</code> is a malicious string with an <code>&lt;img&gt;</code> or another tag, it won’t turn into a real <code>&lt;img&gt;</code> tag. React will escape the content and <em>then</em> insert it into the DOM. So instead of seeing the <code>&lt;img&gt;</code> tag you’ll just see its markup.</p>
...
<p><strong>Well, that wasn’t always true either.</strong> And that’s where <code>$$typeof</code> comes in.</p>
...
<p>While normally you create them with <code>React.createElement()</code>, it is not required. There are valid use cases for React to support plain element objects written like I just did above. Of course, you probably wouldn’t <em>want</em> to write them like this — but this <a href="https://github.com/facebook/react/pull/3583#issuecomment-90296667">can be</a> useful for an optimizing compiler, passing UI elements between workers, or for decoupling JSX from the React package.</p>
...
<p>Why this number specifically? <code>0xeac7</code> kinda looks like “React”.</p></body></html>`
  const portableText = blockTools.htmlToBlocks(HTMLdocument, blockContentType, {
